...
class DnfProvider:
    ...
    def install(self, package_name: str) -> ProviderResult:
        ...
        result = self._shell.run(f"dnf install -y {package_name}")
        if result.success:
